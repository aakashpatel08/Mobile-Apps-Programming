...
public class PhoneMapActivity extends Activity implements OnMapReadyCallback{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mapFragment = (MapFragment) getFragmentManager().findFragmentById(R.id.phoneMap);
            ...
            public boolean onMarkerClick(Marker marker) {
                for (int i = 0; i < MainActivity.departments.length; i++) {
                    if (marker.getTitle().equals(MainActivity.departments[i].getName())) {
                        ...
                        myIntent.setData(Uri.parse("tel:" + MainActivity.departments[i].getPhoneNumber()));
